...
The standard format provides bidirectional conversion between `EntityTag` and the accepted standard
names and symbols.
...
- `to_short_standard_format(entity)` accepts an `EntityTag` and returns the short-standard format
  string
